...
export function BackgroundParticles() {
  ...
  useEffect(() => {
    ...
    const setCanvasSize = () => {
      ...
      const targetWidth = Math.max(1, Math.round(parentRect?.width ?? window.innerWidth));
      const targetHeight = Math.max(1, Math.round(parentRect?.height ?? window.innerHeight));
      width = targetWidth;
      height = targetHeight;
